...
[[{{< fa brands github >}} `saforem2/lattice23`](https://github.com/saforem2/lattice23)]{style="font-size:0.8em;"}
:::
...
- Use these $\left(s_{k}, t_{k}, q_{k}\right)$, $k \in \{x, v\}$ in the
_generalized_ MD update:
<!-- $pink-text: #F06292; -->
...
![](./assets/leapfrog-layer-2D-U1-vertical.light.svg){style="width:70%; text-align:center;"}
...
- [To ensure ergodicity + reversibility[^direction] [^reverse], we split the
[$x$]{.purple-text} update into sequential (complementary)
updates:]{style="font-size:0.9em;"}

  ::: {.callout title="Generalized Update:"}

  1. Update $\textcolor{#07B875}{v}$:  
    [$\textcolor{#07B875}{v'} =$ [$\Gamma_{\theta}^{\pm}$]{.pink-text}$(\textcolor{#AE81FF}{x}, \textcolor{#07B875}{v})$]{style="font-size: 1.2em;"}

  2. Update **half** of $\textcolor{#AE81FF}{x}$:  
    [$\textcolor{#AE81FF}{x'} =$ [$x_{A}$]{.red-text}$\,+\,$[$m_{B}$]{.blue-text}$\,\odot\,$ [$\Lambda_{\theta}^{\pm}$]{.orange-text}$($[$x_{B}$]{.blue-text}$, \textcolor{#07B875}{v'})$]{style="font-size: 1.2em;"}

  3. Update (other) half of $\textcolor{#AE81FF}{x}$:  
    [$\textcolor{#AE81FF}{x''} =$ [$x'_{B}$]{.blue-text}$\,+\,$[$m_{A}$]{.red-text}$\,\odot\,$ [$\Lambda_{\theta}^{\pm}$]{.orange-text}$($[$x'_{A}$]{.red-text}$, \textcolor{#07B875}{v'})$]{style="font-size: 1.2em;"}
...
[^direction]: [We introduce a directional variable $d \sim \mathcal{U}(\pm)$ that is resampled each trajectory to determine the direction of our update]{style="font-size:0.8em;"}
...
Generalized MD update with [$\Lambda_{\theta}^{\pm}$]{.orange-text}, [$\Gamma_{\theta}^{\pm}$]{.pink-text}
:::
...
- Stack gauge links as  
  ...
  [`shape`$\left(U_{\mu}\right)$` = [Nb, 4, Nt, Nx, Ny, Nz, 3, 3]` $\in \mathbb{C}$]{style="font-size:0.85em;"}

  <!-- with `shape`$\left(x_{\mu}\right)$` = [Nb, 2, Nt, Nx, 2]` $\in \mathbb{R}$ -->

- $U$-Network:
    - [$\psi_{\theta}: (U, P) \longrightarrow \left(s_{U},\, t_{U},\, q_{U}\right)$]{.purple-text style="padding:0.2em; border-radius: 6px"}
    <!-- - $\Lambda^{\pm}_{k}(x, v) \rightarrow \left[s^{k}_{x}, t^{k}_{x}, q^{k}_{x}\right]$ -->

- $v$-Network:
    - [$\varphi_{\theta}: (U, P) \longrightarrow \left(s_{P},\, t_{P},\, q_{P}\right)$]{.green-text style="padding:0.2em; border-radius: 6px;"}
...
- $v$-Network:
    - [$\varphi_{\theta}: (U, P) \longrightarrow \left(s_{P},\, t_{P},\, q_{P}\right)$]{.green-text style="border:1px solid #666666; padding:0.2em; border-radius: 6px;"}  
      [$\hspace{50pt}\uparrow$]{.dim-text}  
      [$\hspace{30pt}$ let's look at this]{.dim-text style="padding-top: 0.5em!important;"}
    <!-- - $\Gamma_{k}^{\pm}(x, v) \rightarrow \left[s^{k}_{v}, t^{k}_{v}, q^{k}_{v}\right]$ -->
...
::: {#fig-ridgeplot layout-valign="top"}

![`100`](./assets/SU3/logdet_ridgeplot1.svg)
![`500`](./assets/SU3/logdet_ridgeplot2.svg)
![`1000`](./assets/SU3/logdet_ridgeplot3.svg)

$\log|\mathcal{J}|$ vs. $N_{\mathrm{LF}}$ after `100`, `500`, and `1000` training iterations
:::
...
- This talk: [{{< fa brands github >}} `saforem2/lattice23`](https://github.com/saforem2/lattice23)
- Code repo [{{< fa brands github >}} `saforem2/l2hmc-qcd`](https://github.com/saforem2/l2hmc-qcd)
- Slides [{{< fa solid presentation >}} saforem2.github.io/lattice23](https://saforem2.github.io/lattice23)
